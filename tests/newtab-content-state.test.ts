...
test('newtab exposes a lazy options dashboard iframe route', () => {
  ...
  assert.match(script, /NEWTAB_ADD_SPEED_DIAL_MESSAGE_TYPE/)
  assert.match(script, /function addDashboardBookmarkToActiveSpeedDial\(bookmarkId: string\): Promise<void>/)
  assert.match(script, /isSpeedDialBookmarkPinned\(activeWorkspace\.pinnedIds, normalizedId\)/)
  assert.match(script, /pinnedIds: \[normalizedId, \.\.\.activeWorkspace\.pinnedIds\]/)
