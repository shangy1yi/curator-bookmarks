...
- 当前集成代码状态：在 `29e6a18` 基础上继续追加 options AI 模型 picker listbox 可访问名称修正；报告最新提交以 `git log -1 --oneline` 为准。
